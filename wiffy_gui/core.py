...
def draw_login_forms(frame: ctk.CTkFrame, info_label: WiffyTextLabel, login_button: ctk.CTkButton) -> None:
    try:
        ...
        login_form = LoginEntry(frame, placeholder_text=text.LOGIN_FORM_PLACEHOLDER)
        pwd_form = LoginEntry(frame, show="·", placeholder_text=text.PWD_FORM_PLACEHOLDER)
        forms = {"login": login_form, "pwd": pwd_form}
        save_button = ctk.CTkButton(
            frame,
            text=text.SAVE_USER_DATA_BUTTON_TEXT,
            font=app_settings.base_font,
            height=45,
...
def open_show_songs_menu(info_label: WiffyTextLabel, content_frame: ctk.CTkFrame) -> None:
    ...
    if saved_songs_count:
        ...
        songs_label.pack()
        info_label.configure(text=f"{text.SONGS_FOUND_TEXT} {saved_songs_count}.", text_color="green")
