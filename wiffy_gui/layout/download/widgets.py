...
def create_download_frame_widgets(download_frame: ctk.CTkFrame) -> dict:
    ...
    tracks_info_label = WiffyTextLabel(download_frame, text=text.TRACKS_TO_DOWNLOAD_LABEL)
    spinbox = Spinbox(download_frame, width=120)
    info_dir_label = WiffyTextLabel(download_frame, text=text.CURRENT_DOWNLOAD_FOLDER_LABEL)
    current_dir_label = WiffyTextLabel(download_frame, text=download_path)
    change_dir_button = ctk.CTkButton(download_frame, text=text.CHANGE_DIR_BUTTON_LABEL, font=app_settings.base_font)
    apply_button = ctk.CTkButton(download_frame, text=text.DOWNLOAD_BUTTON_LABEL, font=app_settings.base_font)
    return {
